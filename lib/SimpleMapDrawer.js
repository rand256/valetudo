...
SimpleMapDrawer.drawMap = function(options,callback) {
	...
	const colors = {
		...
		forbidden_mop_marker: 0x42e0ffff,
		forbidden_zone: 0xff000060,
		forbidden_mop_zone: 0x42e0ff80,
	...
	// processing
	const mapData = settings.mapData;
	...
	const drawForbiddenZone = (zones, postfix) => {
		let forbiddenZones = zones.map(zone => {
			...
			for (let i = minx, len = maxx; i < len; i++) {
				for (let j = miny, len2 = maxy; j < len2; j++) {
					if (pointInsideQuadrilateral([i,j],zone[0],zone[1],zone[2],zone[3])) {
						image.setPixelRGBA(i,j,calcOverlayInt(image.getPixelRGBA(i,j),colors['forbidden_' + postfix + 'zone']));
					}
				}
			}
		});
		forbiddenZones.forEach(zone => {
			drawLineByPoints(image,zone.concat([zone[0]]),colors['forbidden_' + postfix + 'marker']);
		});
	};
	if (mapData.forbidden_zones) {
		drawForbiddenZone(mapData.forbidden_zones,'');
	}
	if (mapData.forbidden_mop_zones) {
		drawForbiddenZone(mapData.forbidden_mop_zones,'mop_');
	}
