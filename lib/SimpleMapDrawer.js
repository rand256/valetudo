...
SimpleMapDrawer.drawMap = function(options,callback) {
    ...
    require('zlib').gunzip(settings.gzippedMap, (err, mapBuf) => {
        ...
        let loadRobotImage = decodePNGFromStream(fs.createReadStream(path.join(__dirname, "../client/img/robot.png"))).then(loaded => { robotImage = loaded; });
        ...
        Promise.all([loadChargerImage, loadRobotImage]).then(() => {
            ...
            if (mapData.robot) {
                if (mapData.path) {
                    rotateImage(robotImage,-1 * mapData.path.current_angle - 90);
                }
