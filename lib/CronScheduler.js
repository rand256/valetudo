	...
	timers.concat(ztimers).filter(task => task[0]).forEach(task => {
		...
		.then(_ => {
			return new Promise((resolve,reject) => {
				...
				} else if (task[3].split(',').every(s => !isNaN(+s))) {
...
CronScheduler.prototype.syncTime = function() {
	const timesync = this.configuration.get('ntpd');
	if (!timesync.disabled && os.userInfo().uid === 0) {
		exec('/usr/sbin/ntpd -n -q -p ' + (timesync.server || 'pool.ntp.org'), (error, stdout, stderr) => {
			if (error) {
				console.error(new Date(),"ntpd exec error:", error);
			} else if (stdout === "Alarm clock") {
				setTimeout(() => this.syncTime(),5e3);
			} else {
				console.log(new Date(),"ntpd called:", stderr.trim() || stdout.trim() || "ok");
			}
		});
	}
};
