...
const WebServer = function (options) {
    ...
    this.dummyCloud = options.dummyCloud;
    ...
    this.app.put("/api/start_cleaning_zone_by_name", function (req, res) {
        if (req.body && Array.isArray(req.body)) {
            let name, iterations, zones = [];
            for (let i = 0; i < req.body.length; i++) {
                [name, iterations] = req.body[i];
        ...
        if( req.body != undefined
            ...
            self.vacuum.savePersistentData(persistentData, function(err) {
                if(err) {
                    ...
                    setTimeout(() => { self.dummyCloud.pollMap(); },300);
                    res.status(201).json({message: "ok"});
                }
            });
        } else {
    ...
    self.events.on("valetudo.map", () => {
        ...
        if (!Array.from(wss.clients).some(function some(ws) { return !ws.mapUploadInProgress; })) return;
