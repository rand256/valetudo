...
const WebServer = function (options) {
    ...
    this.app.get("/api/get_telegram_config", function (req, res) {
        ...
        var password = "";
        var clients = [];
        var telegram_send_consumables = true;
        var telegram_send_consumables_every = false;
        if(self.configuration.get("telegram_token")){
            token = self.configuration.get("telegram_token");
            password = self.configuration.get("telegram_password");
            clients = self.configuration.get("telegram_clients");
    ...
    this.app.put("/api/set_telegram_config", function (req, res) {
        ...
        self.configuration.set("telegram_password", req.body.telegram_password);
    ...
    this.app.delete("/api/telegram_client/:clientID", function (req, res) {
        var clients = self.configuration.get("telegram_clients");
        var newclients = [];
        clients.forEach(function (client, index, object) {
            if (client.id == req.params.clientID) object.slice(index, 1);
        });
        self.configuration.set("telegram_clients", newclients);
        self.telegramClient.initiate();
