        ...
        if (req.body && req.body.cron && req.body.id) {
            ...
            .then(() => {
                return new Promise((resolve,reject) => {
                    self.vacuum.addTimer({id: req.body.id.replace(/ /g,"\u{a0}"), cron: req.body.cron, fanpower: +req.body.fanpower || 102, segments: req.body.segments || "", iterations: +req.body.iterations || 1}, function (err, data) {
                        if (err) {
                            reject(err);
                        } else {
                            resolve(data);
