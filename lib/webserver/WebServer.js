	...
	this.app.put("/api/override_queue", function (req, res) {
		if (req.body) {
			...
			if (req.body.postCleaning !== undefined && (req.body.postCleaning > -1 && req.body.postCleaning < 2 || req.body.postCleaning.x !== undefined && req.body.postCleaning.y !== undefined)) {
				self.vacuum.postCleaningOverride = parseInt(req.body.postCleaning) ? req.body.postCleaning : [req.body.postCleaning.x, req.body.postCleaning.y];
				return res.status(201).json({message: "ok"});
			}
		}
	...
	if (this.sslAvailable) {
		...
		httpServer.listen(this.port, function(){
			console.log(new Date(), "Http server running on port " + self.port +
				". Redirects non local requests to https.")
		});
	} else {
