...
const WebServer = function (options) {
    ...
    async function asyncMapUpload(ws) {
        ws.mapUploadInProgress = true;
        while (true) {
            await new Promise((resolve) => {
                ws.send(self.deflatedUploadBuffer, function wsso() {
                    resolve();
                });
            }).catch((err) => { console.error(err); });
            if (!ws.mapUploadEnqueued) break;
            ws.mapUploadEnqueued = false;
        }
    ...
    self.events.on("miio.status", (status) => {
        ...
        wss.clients.forEach(function each2(ws) {
            ws.send(buf, noop);
        });
    });
