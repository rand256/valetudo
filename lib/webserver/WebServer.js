...
const WebServer = function (valetudo) {
	...
	this.app.get("/api/get_fw_version", function (req, res) {
		var version, build, valetudoVersion;
		version = build = valetudoVersion = '?';
		...
		.then(_ => {
			...
			if (packageContent) {
				...
				valetudoVersion = packageContent.version + (packageContent.versionSuffix ? packageContent.versionSuffix : '');
			}
		})
		.catch(_ => {})
		.finally(_ => {
			res.json({
				version: version,
				build: build,
				valetudoVersion: valetudoVersion
			});
		});
	});
