...
const WebServer = function (options) {
    ...
    self.events.on("valetudo.map", () => {
        // don't need to compress anything if there're no clients, or all clients are still in progress
        if (Array.from(wss.clients).every(function some(ws) { return ws.mapUploadInProgress; })) return;
