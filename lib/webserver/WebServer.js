...
const WebServer = function (options) {
    ...
    function prepareStatus(status) {
        ...
        if (status.msg_ver === undefined) status.stateHR = "Disconnected";
        if (status.error_code !== undefined) status.errorHR = Vacuum.GET_ERROR_CODE_DESCRIPTION(status.error_code);
        return status;
    }

    async function asyncMapUpload() {
        self.mapDistributionWorkers++;
        while (true) {
            ...
            await new Promise((resolve,reject) => {
                ...
                ws.send(self.deflatedUploadBuffer, () => {
                    clearTimeout(wst);
                    resolve();
                });
            }).catch(err => {});
    ...
    function asyncMapDistribution() {
        while (self.mapDistributionWorkers <= 5 && self.mapDistributionQueue.length) { // <-- number of simultaneous uploads allowed
            asyncMapUpload();
        }
    }

    self.mapDistributionQueue = [];
    self.mapDistributionWorkers = 0;
    self.events.on("valetudo.map", () => {
        // don't need to compress anything if there're no clients
        if (!wss.clients.size) return;
        // zlib compression on map.parsedData allows to send up to 5x times less data via the network
        zlib.deflate(JSON.stringify({map: self.map.parsedData}), (err, buf) => {
            if (!err) {
                self.deflatedUploadBuffer = buf;
                wss.clients.forEach(ws => {
                    if (!self.mapDistributionQueue.includes(ws)) self.mapDistributionQueue.push(ws);
                });
                asyncMapDistribution();
            }
        });
    });
    ...
    wss.on("connection", (ws) => {
        ...
        if(self.map.parsedData) {
            zlib.deflate(JSON.stringify({map: self.map.parsedData, status: prepareStatus(self.cloud.connectedRobot.status)}), (err, buf) => {
                if (!err) ws.send(buf, noop);
            });
        }
    });
