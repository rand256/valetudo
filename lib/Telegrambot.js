...
Telegrambot.prototype.formatFanPower = function(power) {
    switch (power) {
        case 38:
        case 101: return this.languageGetter('telegram.fanpower.quiet');
        case 60:
        case 102: return this.languageGetter('telegram.fanpower.balanced');
        case 75:
        case 103: return this.languageGetter('telegram.fanpower.turbo');
        case 100:
        case 104: return this.languageGetter('telegram.fanpower.max');
        case 105: return this.languageGetter('telegram.fanpower.mop');
...
{ id: '..numbers..',
  ...
  message:
   { message_id: 1213,
     ...
     chat: { id: ..numbers.., first_name: 'Erinome', type: 'private' },
     date: ..numbers..,
     text: 'some text',
     reply_markup: { inline_keyboard: [Array] } },
  chat_instance: '..numbers..',
  data: 'callback data' }
*/
//console.log(self.lastState)
console.log(query.data);
    let answerCommand = function() { self.answerCallbackQuery(query.id); },
        answerCommandOK = function() { self.answerCallbackQuery(query.id,{text: self.languageGetter('cbqReceivedOK',"Command accepted!")}); },
        answerCommandFailed = function(err) { self.answerCallbackQuery(query.id,{text: err || self.languageGetter('cbqFailed',"Error while running a command!")}); },
        answerCommandWrongState = function() { self.answerCallbackQuery(query.id,{text: self.languageGetter('cbqWrongState',"Command unavailable in current device state!")}); },
    ...
    switch (action[0]) {
        ...
        case "resume":
            if ([5,6,11,16,17,18].includes(self.lastState.state)) {
                ...
                } else if (self.lastState.in_cleaning === 3 && (self.lastState.state === 10 || self.lastState.state === 2)) {
        ...
        case "zones_menu":
            ...
            if (!zones.length) {
                ...
                var msg, subzones, names = [], hashed = {}, chosen = action.slice(1);
                ...
                answerCommand();
                self.editMessageText(query.message.chat.id, query.message.message_id, msg, {reply_markup: self.inlineKeyboardGenerator("zones_menu", zones, chosen)});
            }
            break;
        case "segments_run":
            if (!isV3) {
        ...
        case "segments_menu":
            ...
            .then(res => {
                ...
                var msg, segment, names = [], iterations = 1,
                    chosen = action.slice(1);
                if (chosen.length && chosen[chosen.length-1][0] === 'x') {
                    iterations = +chosen[chosen.length-1][1] || 1;
                    chosen.pop();
                ...
                if (chosen.length) {
                    ...
                    msg += "\n" + self.languageGetter('telegram.selectedSegments', "Currently selected: {{segmentsList}}").replace('{{segmentsList}}',names.join(", "));
                }
                answerCommand();
                self.editMessageText(query.message.chat.id, query.message.message_id, msg, {reply_markup: self.inlineKeyboardGenerator("segments_menu", res, chosen.concat('x' + iterations))});
            })
            .catch(err => {
                console.log(err)
                answerCommandFailed(self.languageGetter('telegram.noSegmentsConfigured',"No rooms available on the map."));
                ...
                MapManager.listStoredMaps((err, maps) => {
                    ...
                    .then(() => {
                        if (self.lastState['lab_status'] === 1) {
                            ...
                            self.deleteMessage(query.message.chat.id, query.message.message_id);
                            self.sendMessage(query.message.chat.id, self.languageGetter('telegram.mapLoadWait',"Map should be reloaded in 30 seconds."));
...
Telegrambot.prototype.inlineKeyboardGenerator = function(type, values, chosen) {
    const self = this;
    let output, back = {text: self.languageGetter('telegram.buttons.back',"-- back --"), callback_data: "main_menu"};
    switch(type) {
        case "power_menu":
            let isV3 = self.lastState.msg_ver === 3;
            output = [
        ...
        case "zones_menu":
            ...
            for (let i = 0; i < values.length; i++) {
                ...
                if (chosen_multi[hash] !== undefined) {
                    ...
                    zone = {text: '['.repeat(chosen_multi[hash]+1) + values[i][0] + ']'.repeat(chosen_multi[hash]+1), callback_data: [type].concat(chosen_next).join(' ')};
                } else {
                    zone = {text: values[i][0], callback_data: [type].concat(chosen, hash).join(' ')};
                }
        ...
        case "segments_menu":
            ...
            for (let i = 0; i < values.length; i++) {
                ...
                if (chosen.includes(values[i].id.toString())) {
                    segment = {text: '[' + values[i].name + ']', callback_data: [type].concat(chosen.filter(s => s !== values[i].id.toString()),'x' + iterations).join(' ')};
                } else {
                    segment = {text: values[i].name, callback_data: [type].concat(chosen,values[i].id,'x' + iterations).join(' ')};
                }
                output[output.length-1].push(segment);
            }
            segment = [];
            for (let i = 1; i <= 3; i++) {
                segment.push({text: (i === iterations ? '[' : '') + " \u{00D7}" + i + " " + (i === iterations ? ']' : ''), callback_data: ["segments_menu"].concat(chosen, 'x' + i).join(' ')});
            }
        ...
        default:
            ...
            output = [
                ...
                [{text: self.languageGetter('telegram.buttons.zones',"Zones"), callback_data: "zones_menu"},{text: self.languageGetter('telegram.buttons.spot',"Spot"), callback_data: "spot_menu"},{text: self.languageGetter('telegram.buttons.rooms',"Rooms"), callback_data: "segments_menu"}],
