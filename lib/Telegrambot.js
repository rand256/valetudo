...
const Telegrambot = function (valetudo) {
	...
	this.events.on("miio.consume_material_notify", (statusData) => {
		if (self.slimbot !== null && self.sendConsumables && (Date.now() - self.lastNotifications.consumables) > 300e3) {
			self.lastNotifications.consumables = Date.now();
	...
	this.events.on("miio.bin_full", (statusData) => {
		if (self.slimbot !== null && self.sendConsumables && (Date.now() - self.lastNotifications.binFull) > 300e3) {
			self.lastNotifications.binFull = Date.now();
			self.clients.forEach(client => {
				if (!client.silence) {
	...
	this.events.on("miio.status", (statusData) => {
		...
		if (self.slimbot !== null && self.lastState.state >= 0 && (self.lastState.state !== res.state || (res.state === 12 && (self.lastState.error_code !== res.error_code)))) {
			let notifyAllowed = false, notifyTypes = self.configuration.get("telegramBot").notifyStatusTypes || 0;
...
Telegrambot.prototype.parseMessage = function(message) {
	...
	if (text[0] === '/setme') {
		...
		if ((client = self.clients.findIndex(client => client.id === message.chat.id)) >= 0 && JSON.stringify(self.clients[client]) !== JSON.stringify(message.chat)) {
			let silenced = self.clients[client].silence;
			self.clients[client] = message.chat;
			if (silenced) {
				self.clients[client].silence = silenced;
			}
			self.saveClients();
		}
