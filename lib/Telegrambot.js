...
const Telegrambot = function (options) {
    ...
    this.events.on("miio.bin_full", (statusData) => {
        ...
        if (self.slimbot !== null && self.sendConsumables && (Date.now() - this.lastNotifications.binFull) > 300e3) {
            ...
            self.clients.forEach(client => {
                if (!client.silence) {
                    self.sendMessage(client.id, self.languageGetter('telegram.binFull', "Dust bin congestion detected, cleaning up required!"));
                }
            });
        }
    });
...
Telegrambot.prototype.formatFanPower = function(power) {
    switch (power) {
        case 101: return this.languageGetter('home.fanspeedPresets.quiet');
        case 102: return this.languageGetter('home.fanspeedPresets.balanced');
        case 103: return this.languageGetter('home.fanspeedPresets.turbo');
        case 104: return this.languageGetter('home.fanspeedPresets.max');
        case 105: return this.languageGetter('home.fanspeedPresets.mop');
    ...
    switch (text[0]) {
        ...
        case "/start":
            switch (parameters[0]) {
                ...
                case "/clean":
                case "clean": self.sendMessage(message.chat.id, self.languageGetter('telegram.help.clean',"/clean - if you specify room or zone names separated with plus signs, will start rooms or zoned cleaning. Otherwise will start full cleaning. Rooms and zones can't be mixed.\n\nYou can also specify a number of cleaning passes for zoned cleaning using `xNumber` postfix. In example, `/clean Zone1 x2 + Zone2 x1 + Zone3` will start zoned cleaning with `Zone1` subzones cleaned twice, `Zone2` subzones cleaned once and `Zone3` subzones cleaned the number of times speficied in the zone description. For rooms you can specify the number of passes only once.\n\nNote you can't clean more that 5 subzones at once."),{parse_mode: "Markdown"}); break;
                case "/stop":
                ...
                case "/power":
                case "power": self.sendMessage(message.chat.id, self.lastState.msg_ver === 3 ? self.languageGetter('telegram.help.powerV3',"/power - specifies fan power profile: *Quiet*, *Balanced*, *Turbo*, *Max* (not recommended!) or *Mop*") : self.languageGetter('telegram.help.power',"/power - specifies current fan power in range 1 to 100, or special value 105 for mop mode for robot supporting it.\n\n38 - Quiet\n60 - Balanced\n75 - Turbo\n100 - Max (not recommended!)")); break;
                case "/rooms":
                case "rooms": self.sendMessage(message.chat.id, self.languageGetter('telegram.help.segments',"/rooms - will list avalable room names that could be used in room cleaning.")); break;
                case "/zones":
        ...
        case "/clean":
            if ([2,3,8,10].includes(self.lastState.state)) { // todo: in which modes it's possible to start new full cleaning?
                ...
                // try zoned or segments cleaning if there are parameters. at first - zones
                ...
                for (let times, timesMatch, name, i = 0; i < parameters.length; i++) {
                    times = 0;
                    name = parameters[i];
                    if (timesMatch = name.match(/ (?:x|х)([1-9]+)/,'')) {
                        if (timesMatch[1] > 3) {
                            times = 3;
                    ...
                    if ((zone = zonesAvailable.find(item => item[0].trim().toLowerCase() === name.toLowerCase())) !== undefined) {
                        ...
                        zoneList.push(msg);
                    } else if (self.lastState.msg_ver === 3) {
                        // then check segments if any requested parameter wasn't find as a zone
                        var segment, segments = [], segmentList = []
                        self.getActualSegments().then(res => {
                            let totalTimes = 1;
                            for (i = 0; i < parameters.length; i++) {
                                ...
                                name = name.trim();
                                if ((segment = res.find(item => item.name && (item.name.trim().toLowerCase() === name.toLowerCase()) || item.id === +name)) !== undefined) {
                                    let msg = '"' + (segment.name || '#' + segment.id).trim() + '"';
                                    segments.push(segment.id);
                                    segmentList.push(msg);
                                } else {
                                    self.sendMessage(message.chat.id, self.languageGetter('telegram.segmentNotFound',"Can't find room by name \"{{segmentName}}\".")
                                        .replace('{{segmentName}}', (name === undefined ? '#' + name : name))
                                    );
                                    message.text = "/rooms";
                            ...
                            // start segmented cleaning
                            self.sendMessage(message.chat.id,self.languageGetter('telegram.performingSegments',"Going to room cleaning:\n{{segmentNames}}").replace('{{segmentNames}}',segmentList.join(', ') + (totalTimes > 1 ? ' (x' + totalTimes + ')' : '')));
                            if (self.sendConsumablesEvery && self.clients.some(client => client.id === message.chat.id && client.silence !== true)) {
                                self.vacuum.getConsumableStatus(function (err, res) {
                                    if (err) {
                                        console.log(err);
                            ...
                            console.log([segments,totalTimes])
                            self.vacuum.startCleaningSegment([segments,totalTimes],function (err, data) {
                                if (err) {
                                    console.log(err);
                                    self.sendMessage(message.chat.id, self.languageGetter('telegram.errorReply',"There was an error: {{error}}").replace('{{error}}',err));
                                }
                        ...
                        .catch(err => {
                            console.log(err)
                            self.sendMessage(message.chat.id, self.languageGetter('telegram.noRoomsConfigured',"No rooms available on the map."));
                        });
                        return;
                    } else {
        ...
        case "/power":
            ...
            if (!parseInt(speed) || (self.lastState.msg_ver === 3 && (speed < 101 || speed > 105)) || (self.lastState.msg_ver !== 3 && (speed <= 0 || speed > 100) && speed !== 105)) {
                self.sendMessage(message.chat.id, self.lastState.msg_ver === 3 ? self.languageGetter('telegram.wrongPowerV3',"You need to specify fanspeed value:\n101 - Quiet\n102 -Balanced\n103 - Turbo\n104 - Max (not recommended!)\b105 - Mop") : self.languageGetter('telegram.wrongPower',"Allowed fan speed could be in range from 1 to 100 (plus special value 105 for mop mode if applicable)."));
                return;
            }
        ...
        case "/rooms":
            ...
            .then(res => {
                ...
                var msg = self.languageGetter('telegram.segments',"List of available rooms:");
                res.forEach(segment => {
                    msg += self.languageGetter('telegram.segmentsElement',"\n* {{segmentName}}")
                        .replace('{{segmentName}}',segment.name !== undefined ? segment.name + ' (#' + segment.id + ')' : '#' + segment.id);
                });
        ...
        case "/status":
            self.vacuum.getCurrentStatus(function(err, res) {
                if (!err) {
                    ...
                    msg += "\n" + self.languageGetter('telegram.power',"Power: ") + (res.fan_power !== 105 ? self.formatFanPower(res.fan_power) : self.languageGetter('telegram.mop',"Mop Mode"));
                    msg += "\n" + self.languageGetter('telegram.battery',"Battery: ") + res.battery + "%";
                    self.sendMessage(message.chat.id, msg);
                    Object.assign(self.lastState, res);
                } else {
        ...
        case "/maps":
            ...
            break;
        default:
            self.sendMessage(message.chat.id, self.languageGetter('telegram.unknownCommand',"Unknown command."));
            break;
    }
}
...
Telegrambot.prototype.getActualSegments = function() {
    ...
    .then(res => {
        ...
        if (res.image && res.image.segments && res.image.segments.id) {
            segments = res.image.segments.id.map(id => { return {id: id, name: segmentNames[id]}; });
        }
