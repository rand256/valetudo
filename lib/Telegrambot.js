...
Telegrambot.prototype.parseCallbackQuery = function(query) {
	...
	switch (action[0]) {
		...
		case "resume":
			...
			break;
		case "stop":
			if ([1,2,5,12,17,18].includes(self.lastState.state) || ([8,10].includes(self.lastState.state) && self.lastState.in_cleaning)) {
				answerCommandOK();
		...
		case "home":
			if ([2,3,5,10,11,12,16,17,18].includes(self.lastState.state)) {
				answerCommandOK();
				if ([5,11,17,18].includes(self.lastState.state) && self.lastState.msg_ver !== 3) {
					self.vacuum.stopCleaning(function (err, data) {
						self.vacuum.driveHome(function (err, data) {
							sendAfterCommandSequence();
						});
