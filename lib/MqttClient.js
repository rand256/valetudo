			...
			this.vacuum.getCurrentStatus((err, res) => {
				if (!err) {
					if (res.in_cleaning === 2 && [2,10,12].includes(res.state)) {
						...
						this.vacuum.resumeCleaningSegment(() => {
						});
					} else {
	...
	if (msg && msg.command) {
		switch (msg.command) {
			...
			case CUSTOM_COMMANDS.STORE_MAP:
				if (msg.name) {
					this.MapManager.storeMap(msg.name, function(err, data) {
			...
			case CUSTOM_COMMANDS.LOAD_MAP:
				if (msg.name) {
					this.mapManager.loadMap(msg.name, function (err, data) {
						if (err) { console.info(new Date(),err); }
					});
				} else {
			...
			case CUSTOM_COMMANDS.GET_DESTINATIONS:
				...
				.catch(e => console.error(new Date(),"Getting destinations for MQTT failed:", e));
				break;
			default:
