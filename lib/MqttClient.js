	...
	if (this.client && this.client.connected === true) {
		...
		.then(_ => {
			...
			if (binInTime < this.bin_in_time) {
				let cdt = new Date();
				this.last_bin_out = cdt.getTime();
				try { try { fs.utimesSync(this.bin_mark_file, cdt, cdt); } catch (err) { fs.closeSync(fs.openSync(this.bin_mark_file, 'w')); }; } catch (err) { this.publishCommandStatus("update_attributes_topic",null,err); };
			}
