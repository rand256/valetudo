...
const MqttClient = function (valetudo) {
	...
	this.autoconf_payloads = {
		vacuum: {
			...
			schema: "state",
			device: { manufacturer: "Roborock", model: (this.vacuum.model === "roborock.vacuum.s5" ? "S5" : "v1"), name: this.identifier, identifiers: [ this.identifier ], sw_version: valetudo.version },
			supported_features: [
				"start",
				"pause",
				"stop",
	...
	this.bin_mark_file = '/mnt/data/valetudo/last_bin_out.0';
	this.bin_in_time = 0;
	try { this.last_bin_out = +fs.statSync(this.bin_mark_file).mtime; } catch (err) { this.last_bin_out = -1; }
...
MqttClient.prototype.stop = async function () {
	if (this.client) {
		...
		await new Promise(res => this.client.end(true,() => { this.client = null; res(); }));
	}
};
MqttClient.prototype.reload = function () {
	this.stop();
	this.connect();
};
	...
	if (this.client && this.client.connected === true) {
		...
		.then(_ => {
			...
			if (binInTime < this.bin_in_time) {
				this.last_bin_out = Date.now();
				try { try { fs.utimesSync(this.bin_mark_file, Date.now(), Date.now()); } catch (err) { fs.closeSync(fs.openSync(this.bin_mark_file, 'w')); }; } catch (err) { this.publishCommandStatus("update_attributes_topic",null,err); };
			}
			this.bin_in_time = binInTime;
...
MqttClient.prototype.handleCustomCommand = function (message) {
	...
	if (msg && msg.command) {
		switch (msg.command) {
			 ...
			 *	 }
			 *   ],
			 *   "afterCleaning": "Base or Stop or YourSavedGotoName" (optional)
			 * }
			 */
			case CUSTOM_COMMANDS.ZONED_CLEANUP:
				if (Array.isArray(msg.zone_ids)) {
					...
					if (zones.length) {
						...
						this.publishCommandStatus(msg.command,null,"Invalid zone_ids");
					}
				} else {
					this.publishCommandStatus(msg.command,null,"Missing zone_ids");
				}
				break;
			/**
			 * {
			...
			case CUSTOM_COMMANDS.GO_TO:
				if (msg.spot_id) {
					...
					this.publishCommandStatus(msg.command,null,"Missing spot_id");
				}
				break;
			/**
			 * {
			...
			case CUSTOM_COMMANDS.GET_DESTINATIONS:
				...
				new Promise((resolve,reject) => {
					this.vacuum.getCurrentStatus((err, data) => {
						if (err || data.msg_ver !== 3 || !this.map.bin) {
							return resolve(null);
						}
						this.vacuum.getRoomMapping((err, data) => {
							if (err) {
			...
			case CUSTOM_COMMANDS.PLAY_SOUND:
				let cmd = null;
				if (fs.existsSync('/usr/bin/sox') && (msg.file && fs.existsSync(msg.file) || msg.location)) {
					cmd = '/usr/bin/sox ' + (msg.volume ? '-v ' + msg.volume + ' ' : '') + '"' + (msg.location || msg.file) + '" -d';
				} else if (fs.existsSync('/usr/bin/uart_test') && msg.file && fs.existsSync(msg.file) && msg.file.endsWith('.wav')) {
