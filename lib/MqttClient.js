		...
		switch (msg.command) {
			...
			case CUSTOM_COMMANDS.PLAY_SOUND:
				if (msg.file && fs.existsSync(msg.file)) {
					if (!fs.existsSync('/usr/bin/uart_test')) {
						return this.publishCommandStatus(msg.command,null,"Missing uart_test tool on the device");
					}
					exec('/usr/bin/uart_test -e ' + msg.file, (error, stdout, stderr) => {
						if (!error) {
							this.publishCommandStatus(msg.command,'ok',null);
						} else {
