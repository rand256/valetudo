...
MqttClient.prototype.updateMapDataTopic = function (mapDTO) {
    if (this.client && this.client.connected === true && mapDTO && mapDTO.parsedData) {
        if (!this.isPublishingMap) {
            ...
            this.client.publish(this.topics.map_data, JSON.stringify(mapDTO.parsedData), {retain: true, qos:this.qos}, err => {
                this.isPublishingMap = false;
                if (this.nextMapToPublish) {
                    this.updateMapDataTopic(JSON.parse(this.nextMapToPublish));
                    this.nextMapToPublish = null;
                }
            });
        } else {
            this.nextMapToPublish = JSON.stringify(mapDTO);
        }
    }
};
