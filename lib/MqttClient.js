			...
			if (!err) {
				...
				this.vacuum.getCleanSummary((err, res2) => {
					if (!err) {
						...
						this.publishCommandStatus("update_attributes_topic",null,err);
						this.attributesUpdateTimeout = setTimeout(() => {
							this.updateAttributesTopic()
						}, this.attributesUpdateInterval);
					}
...
MqttClient.prototype.handleCommand = function (command) {
	switch (command) { //TODO: error handling
		...
		case MQTT_COMMANDS.RETURN_TO_BASE:
			this.vacuum.driveHome((err, data) => this.publishCommandStatus(command,data,err));
			break;
		...
		switch (msg.command) {
			...
			case CUSTOM_COMMANDS.ZONED_CLEANUP:
				if (Array.isArray(msg.zone_ids)) {
					...
					if (zones.length) {
						this.vacuum.startCleaningZone(zones, (err, data) => this.publishCommandStatus(msg.command,data,err));
					} else {
				...
				if (Array.isArray(msg.segment_ids)) {
					...
					this.vacuum.getRoomMapping((err, data) => {
						...
						if (segments.length) {
							...
							this.vacuum.startCleaningSegment([segments, repeats], (err, data) => this.publishCommandStatus(msg.command,data,err));
						} else {
			...
			case CUSTOM_COMMANDS.GET_DESTINATIONS:
				let result = {
					spots: this.configuration.get("spots").map(spot => spot[0]).sort(),
					zones: this.configuration.get("areas").map(zone => zone[0]).sort(),
					rooms: [],
					updated: Date.now()
				};
						...
						this.vacuum.getRoomMapping((err, data) => {
							...
							require('zlib').gunzip(this.map.bin, (err, mapBuf) => {
								...
								if (mapData.image && mapData.image.segments && mapData.image.segments.id) {
									segments = mapData.image.segments.id.map(id => segments[id] || ('#' + id));
								}
								resolve(segments.sort());
							});
						});
					});
				})
			...
			case CUSTOM_COMMANDS.PLAY_SOUND:
				if (msg.file && fs.existsSync(msg.file)) {
					...
					if (fs.existsSync('/usr/bin/sox')) {
						cmd = '/usr/bin/sox ' + (msg.volume ? '-v ' + msg.volume + ' ' : '') + '"' + msg.file + '" -d';
					} else if (fs.existsSync('/usr/bin/uart_test')) {
						cmd = '/usr/bin/uart_test -e "' + msg.file + '"';
					} else {
						return this.publishCommandStatus(msg.command,null,"Missing both uart_test and sox tools on the device, can't play sounds.");
					}
					...
					this.publishCommandStatus(msg.command,null,"File not specified or doesn't exist.");
				}
				break;
			default:
