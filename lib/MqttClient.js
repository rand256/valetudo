...
MqttClient.prototype.updateStatusTopic = function (statusData) {
	if (this.client && this.client.connected === true && statusData.battery !== undefined) {
		...
		if (this.vacuum.features.water_box_status) {
			...
			response.water_box_status = "" + (waterBoxStatusTexts[statusData.water_box_status] || `Unknown ${statusData.water_box_status}%`);
		}
		if (this.vacuum.features.mop_install_status) {
			var mopStatusTexts = {
				0: "Carriage not installed",
				1: "Carriage installed",
			};
			response.water_box_carriage_status = "" + (mopStatusTexts[statusData.water_box_carriage_status] || `Unknown ${statusData.water_box_carriage_status}%`);
		}
		if (this.vacuum.features.water_usage_ctrl) {
			...
			response.water_box_mode = (waterGradePresets[statusData.water_box_mode] || `Custom ${statusData.water_box_mode}%`);
		}
