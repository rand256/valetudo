 ...
 */
MqttClient.prototype.asyncUpdateMapDataTopic = async function(mapDataStr) {
    this.isPublishingMap = true;
    while (true) {
        await new Promise((resolve) => {
            this.client.publish(this.topics.map_data, mapDataStr, {retain: true, qos:this.qos}, err => {
                resolve();
            });
        }).catch((err) => { console.error(err); });
        if (!this.nextMapToPublish) break;
        mapDataStr = this.nextMapToPublish;
        this.nextMapToPublish = null;
    }
    this.isPublishingMap = false;
}
...
MqttClient.prototype.updateMapDataTopic = function (mapDTO) {
    if (this.client && this.client.connected === true && mapDTO && mapDTO.parsedData) {
        if (!this.isPublishingMap) {
            this.asyncUpdateMapDataTopic(JSON.stringify(mapDTO.parsedData));
        } else {
            this.nextMapToPublish = JSON.stringify(mapDTO.parsedData);
        }
    }
};
