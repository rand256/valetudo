...
MapManager.prototype.loadMap = function(name, callback) {
	...
	.then(res => {
		// restore native timers (if any) a bit later after the map was restored
		if (loadedOpts && loadedOpts.timers) {
			setTimeout(() => {
				this.vacuum.getTimers((err,data) => {
					...
					let tPromises = [];
					data.forEach(t => {
						tPromises.push(new Promise((res,rej) => {
							self.vacuum.deleteTimer(t.id, (err,data) => {
								if (err) {
									rej(err);
								} else {
									res();
					...
					.then(res => {
						...
						loadedOpts.timers.forEach(t => {
							tPromises.push(new Promise((res,rej) => {
								self.vacuum.addTimer({id: t[1].replace(/ /g,"\u{a0}"), cron: t[2], fanpower: +t[5], segments: t[3], iterations: +t[4]}, (err,data) => {
									if (err) {
										rej(err);
									} else {
										res();
					...
					.catch(e => console.log(new Date(),"can't process native timers at map restore:",e));
				});
			}, res === "ok" ? 1e3 : 30e3);
		}
		return res;
