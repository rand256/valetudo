...
const Vacuum = function(valetudo) {
	...
	this.events.on("miio.status_changed", (status) => {
		if (![2,6,10,12,17,18].includes(status.state)) {
			self.resetCleaningQueues();
		}
	...
	this.events.on("miio.cleaning_finished", (msg) => {
		if (self.zonedCleaningQueue.length && ["event.zoned_clean_succ","event.zoned_clean_partial_done"].includes(msg.method)) {
			self.startCleaningZone(null,function(){}); // process possibly enqueued zones
			return;
		} else if (self.segmentedCleaningQueue.length && ["event.segment_clean_succ","event.segment_clean_partial_done"].includes(msg.method)) {
			self.startCleaningSegment(null,function(){}); // process possibly enqueued segments
			return;
		} else {
			self.resetCleaningQueues();
		}
		const dest = self.configuration.get("system").afterCleanDest,
...
Vacuum.prototype.resetCleaningQueues = function() {
	...
	this.getCurrentStatus((err,res) => {
		if (!err) {
			if (Array.isArray(newZones) && Array.isArray(newZones[0]) && newZones[0].length > 0) {
				if (res.state === 17) {
					this.zonedCleaningQueue = this.zonedCleaningQueue.concat(newZones);
					callback(null,['enqueued']);
					return;
				} else {
					this.resetCleaningQueues();
					this.zonedCleaningQueue = newZones;
				}
			}
	...
	this.getCurrentStatus((err,res) => {
		if (!err) {
			if (Array.isArray(newSegments) && Array.isArray(newSegments[0]) && newSegments[0].length > 0) {
				if (res.state === 18) {
					this.segmentedCleaningQueue.push(newSegments);
					callback(null,['enqueued']);
					return;
				} else {
					this.resetCleaningQueues();
					this.segmentedCleaningQueue = [newSegments];
				}
			}
