...
Vacuum.prototype.getCleanRecordMap = function (recordId, callback) {
    var self = this;
    var timed_callback = function() { setTimeout(() => { callback(null, "ok"); }, 500) };
    this.sendMessage("get_clean_record_map", [parseInt(recordId)], {}, function (err, response) {
        if (!err && response) {
            switch (response[0]) {
            case "retry":
                setTimeout(function() {
                    self.sendMessage("get_clean_record_map", [parseInt(recordId)], {}, function (err, response) {
                        if (!err) {
                            if (response && response[0] == "map_upload_handler") {
                                timed_callback();
                            } else {
                                callback("map not ready yet: " + JSON.stringify(response));
                            }
                        } else {
                            callback(err);
                        }
                ...
                break;
            case "map_upload_handler":
                timed_callback();
                break;
            default:
                console.log("unknown response to getCleanRecordMap:", response);
