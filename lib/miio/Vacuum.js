...
const Vacuum = function(valetudo) {
	...
	this.events.on("miio.cleaning_finished", () => {
		...
		let notV1 = (self.model !== 'rockrobo.vacuum.v1');
		if (notV1) {
			...
			if (self.zonedCleaningQueue.length) {
				self.pauseCleaning((err,data) => {
					setTimeout(() => self.startCleaningZone(null,() => {}),9e2);
				});
				return;
			} else if (self.segmentedCleaningQueue.length) {
				self.pauseCleaning((err,data) => {
					setTimeout(() => self.startCleaningSegment(null,() => {}),9e2);
				});
				return;
			}
		}
		self.resetQueues(0x0f);
		if (dest === 1 || isSpot) {
			self[notV1 ? 'stopCleaning' : 'pauseCleaning']((err,data) => {
				if (!err && isSpot) {
