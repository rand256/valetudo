...
Vacuum.prototype.addTimer = function(settings, callback) {
    this.getCurrentStatus((err,res) => {
        if (!err && res.msg_ver === 3) {
            this.sendMessage("set_timer", [[settings.id, [settings.cron, ["start_clean", {"fan_power": settings.fanpower, "segments": settings.segments, "repeat": settings.iterations, "clean_order_mode": 0}]]]], {}, callback);
        } else {
...
/* segments - array: [[id1, id2, ..], iterations, clean_order_mode]
    it's not completely clear what the last parameter means
 */
Vacuum.prototype.startCleaningSegment = function(segments, callback) {
    if(Array.isArray(segments) && Array.isArray(segments[0]) && segments[0].length > 0) {
        this.sendMessage("app_segment_clean", [{segments: segments[0], repeat: segments[1] || 1, clean_order_mode: segments[2] || 0}], {}, callback)
    } else {
