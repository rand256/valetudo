	...
	this.socket.on("message", (incomingMsg, rinfo) => {
		...
		if (decodedResponse.msg === null) {
			if(decodedResponse.stamp === 0) { //Initial TimeSync Packet, but this might be missing
				...
				console.info(new Date(), "Robot (re)connected");
			...
			if (decodedResponse.msg.method) {
				switch (decodedResponse.msg.method) {
					case "props":
						this.updateState(decodedResponse.msg.params);
						break;
					case "event.status":
						if(
...
Dummycloud.prototype.updateState = function(state) {
	if (!this.statusLocked) {
		Object.assign(this.connectedRobot.status, state);
		this.events.emit("miio.status", this.connectedRobot.status);
	}
	if ([4,5,6,7,11,15,16,17,18].includes(this.connectedRobot.status.state)) {
		this.pollMap();
	}
};
...
Dummycloud.prototype.pollMap = function(retry) {
	...
	let timeout = [4,5,6,7,11,15,16,17,18].includes(this.connectedRobot.status.state) ? 1000 : 60000;
	const now = new Date();
	if (now - 900 > this.lastMapPoll || retry) {
		...
		timeout = 900 - (now - this.lastMapPoll);
	}
